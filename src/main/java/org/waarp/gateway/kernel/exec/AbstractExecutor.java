 ...
 * will be refused for execution. If
 * "REFUSED" is set, the command "RETR" or "STOR" like operations will be
 * stopped at starting of command.<br> If the
 * command starts with "EXECUTE", the following will be a command to be
 * executed.<br> If the command starts with
 * "JAVAEXECUTE", the following will be a command through Java class to be
 * executed.<br> If the command starts with
 * "R66PREPARETRANSFER", the following will be a r66 prepare transfer execution
 * (asynchronous operation only).<br>
 * <p>
 * <p>
 * The following replacement are done dynamically before the command is
 * executed:<br> - #BASEPATH# is replaced by the
 * full path for the root of FTP Directory<br> - #FILE# is replaced by the
 * current file path relative to FTP Directory
 * (so #BASEPATH##FILE# is the full path of the file)<br> - #USER# is replaced
 * by the username<br> - #ACCOUNT# is
 * replaced by the account<br> - #COMMAND# is replaced by the command issued for
 * the file<br> - #SPECIALID# is replaced
 * by the FTP id of the transfer (whatever in or out)<br> - #UUID# is replaced
 * by a special UUID globally unique for the
 * transfer, in general to be placed in -info part (for instance ##UUID## giving
 * #uuid#)<br>
 *
...
public abstract class AbstractExecutor {
  ...
  public static void initializeExecutor(String retrieve, long retrDelay,
    ...
    logger.info("Executor configured as [RETR: " +
                commandExecutor.getRetrType() + ":" + commandExecutor.pretrCMD +
                ":" + commandExecutor.pretrDelay +
                ":" +
                commandExecutor.pretrRefused +
                "] [STOR: " + commandExecutor.getStorType() + ":" +
                commandExecutor.pstorCMD + ":" +
                commandExecutor.pstorDelay + ":" +
                commandExecutor.pstorRefused + "]");
  }
   ...
   *     FilePath(relative to BaseDir) Command"
  ...
  /**
   * @param command
   * @param args as {User, Account, BaseDir, FilePath(relative to BaseDir),
   *     Command}
   *
  ...
  public static String getPreparedCommand(String command, String[] args) {
    ...
    logger.debug(
        "Will replace value in " + command + " with User=" + args[0] + ":Acct="
        + args[1] + ":Base=" + args[2] + ":File=" + args[3] + ":Cmd=" +
        args[4]);
