 ...
 * MDTM,MLSD,MLST,SIZE,AUTH) <br> 9) BINARY (binary format)<br> 10) if get =>
 * RETR filepath.basename ; if put => STOR
 * filepath ; if append => APPE filepath.basename<br> 11) if digest &
 * get/put/append & remote site compatible with
 * XCRC,XMD5,XSHA1 => FEAT (parsing if found corresponding XCRC,XMD5,XSHA1) ;
 * then XCRC/XMD5/XSHA1 filepath.basename ;
 * then locally comparing this XCRC/XMD5/XSHA1 with the local file<br> 12) if
 * post => extraCommand2 with ',' replaced by
 * ' ' (note: do not use standard commands from FTP like
 * ACCT,PASS,REIN,USER,APPE,STOR,STOU,RETR,RMD,RNFR,RNTO,ABOR,CWD,CDUP,MODE,PASV,PORT,STRU,TYPE
