    ...
    protected static boolean getParams(String[] args, boolean submitOnly) {
        ...
        if (rhost != null && rule != null && localFilename != null) {
            ...
            try {
                ...
                logger.error(
                        Messages.getString("AbstractBusinessRequest.NeedMoreArgs", "(-to -rule -file | -to -id)")
                        //$NON-NLS-1$
                        , e);
                return false;
            }
        ...
        logger.error(Messages.getString("AbstractBusinessRequest.NeedMoreArgs", "(-to -rule -file | -to -id)") +
                     //$NON-NLS-1$
                     _INFO_ARGS);
        return false;
    }
