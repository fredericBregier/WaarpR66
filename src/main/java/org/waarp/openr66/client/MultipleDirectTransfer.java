        ...
        for (String filename : localfilenames) {
            if (!(filename.contains("*") || filename.contains("?") || filename.contains("~"))) {
                ...
                RequestInformation info = new RequestInformation(futureInfo, requested, rule, filename,
                                                                 (byte) InformationPacket.ASKENUM.ASKLIST.ordinal(), -1,
            ...
            for (String filename : localfilenames) {
                if (!(filename.contains("*") || filename.contains("?") || filename.contains("~"))) {
                    ...
                    try {
                        ...
                        logger.warn(
                                Messages.getString("Transfer.14") + filename + " : " + e.getMessage()); //$NON-NLS-1$
                    }
                }
            }
        }
                ...
                for (String filename : files) {
                    ...
                    if (filename != null && !filename.isEmpty()) {
                        ...
                        OutputFormat outputFormat = new OutputFormat("Unique "
                                                                     + MultipleDirectTransfer.class.getSimpleName(),
                                                                     null);
                        if (future.isSuccess()) {
