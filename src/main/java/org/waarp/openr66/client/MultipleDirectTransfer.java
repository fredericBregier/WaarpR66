 ...
 * for multiple files to multiple hosts at
 * once.<br> Files will have to be separated by ','.<br> Hosts will have to be
 * separated by ','.<br>
 * <br>
 * For instance: -to host1,host2,host3 -file file1,file2 <br> Will generate:
 * <br> -to host1 -file file1<br> -to host1
 * -file file2<br> -to host2 -file file1<br> -to host2 -file file2<br> -to host3
 * -file file1<br> -to host3 -file
 * file2<br>
 *
...
public class MultipleDirectTransfer extends DirectTransfer {
      ...
      if (future.isSuccess()) {
        ...
        outputFormat
            .setValue(
                ...
                "Multiple " + Messages.getString("Transfer.Status") +
  ...
  public static List<String> getLocalFiles(DbRule dbrule,
    ...
    if (localfilenames != null) {
      for (String filename : localfilenames) {
        if (!(filename.contains("*") || filename.contains("?") ||
          ...
          // local: must check
          logger
              .info("Local Ask for " + filename + " from " + dir.getFullPath());
          List<String> list;
          try {
            ...
            logger.warn(
                Messages.getString("Transfer.14") + filename + " : " +
  ...
  public static List<String> getRemoteFiles(DbRule dbrule,
                                            String[] localfilenames,
                                            String requested,
                                            NetworkTransaction networkTransaction) {
    List<String> files = new ArrayList<String>();
    for (String filename : localfilenames) {
      if (!(filename.contains("*") || filename.contains("?") ||
        ...
        logger.info(Messages.getString("Transfer.3") + filename + " to " +
                    requested); //$NON-NLS-1$
        RequestInformation info =
            new RequestInformation(futureInfo, requested, rule, filename,
                                   (byte) InformationPacket.ASKENUM.ASKLIST
                                       .ordinal(), -1,
        ...
        if (futureInfo.isSuccess()) {
          ...
          logger.error(
              Messages.getString("Transfer.6") + filename + " to " + requested +
              ": " +
              (futureInfo.getCause() == null? "" :
