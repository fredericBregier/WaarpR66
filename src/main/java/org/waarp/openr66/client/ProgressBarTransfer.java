 ...
 * enabling access to statistic of the
...
public abstract class ProgressBarTransfer extends AbstractTransfer {
      ...
      try {
        ...
        if ((!localChannelReference.getFutureValidRequest().isSuccess()) &&
          ...
          if (runner
            ...
            try {
              Thread
                  .sleep(Configuration.configuration.getConstraintLimitHandler()
                                                    .getSleepTime());
            } catch (InterruptedException e) {
            }
            i--;
   ...
   * INTERVALCALLBACK). Note that final rank is
   * unknown.
   *
