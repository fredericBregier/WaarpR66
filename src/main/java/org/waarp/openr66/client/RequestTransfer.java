...
public class RequestTransfer implements Runnable {
  protected static String _INFO_ARGS =
      Messages.getString("RequestTransfer.0") +
  ...
  protected static boolean getParams(String[] args) {
    _INFO_ARGS = Messages.getString("RequestTransfer.0") +
                 Messages.getString("Message.OutputFormat"); //$NON-NLS-1$
    if (args.length < 5) {
      logger
          .error(_INFO_ARGS);
    ...
    if ((scancel && srestart) || (scancel && sstop) || (srestart && sstop)) {
      logger.error(
          Messages.getString("RequestTransfer.15") + _INFO_ARGS); //$NON-NLS-1$
      return false;
    }
    if (sspecialId == DbConstant.ILLEGALVALUE || srequested == null) {
      logger.error(
          Messages.getString("RequestTransfer.16") + _INFO_ARGS); //$NON-NLS-1$
      return false;
    }
  ...
  public void run() {
    ...
    try {
      ...
      if (futureInfo.isSuccess()) {
        ...
        try {
          ...
          if (runner.isAllDone()) {
            logger
                .error(Messages.getString("RequestTransfer.21")); //$NON-NLS-1$
            future.setResult(
                new R66Result(new OpenR66DatabaseGlobalException(e), null, true,
                              ErrorCode.Internal, null));
            future.setFailure(e);
          ...
          future.setResult(
              new R66Result(new OpenR66DatabaseGlobalException(e1), null, true,
                            ErrorCode.Internal, null));
          future.setFailure(e);
          return;
        }
    ...
    if (cancel || stop || restart) {
      ...
      future
          .setResult(new R66Result(null, true, runner.getErrorInfo(), runner));
      future.setSuccess();
    }
  }
  ...
  private ErrorCode sendStopOrCancel(DbTaskRunner runner, byte code) {
    ...
    host = R66Auth.getServerAuth(DbConstant.admin.getSession(),
                                 this.requester);
    if (host == null) {
      logger.error(Messages.getString("RequestTransfer.39") +
                   this.requester); //$NON-NLS-1$
      OpenR66Exception e =
