...
public class FileBasedConfiguration {
   ...
   * will enable Thrift support on the TCP
      ...
      if (useCpuLimit || highcpuLimit > 0) {
        if (highcpuLimit > 0) {
          ...
          config.setConstraintLimitHandler(
              new R66ConstraintLimitHandler(useCpuLimit, useCpuLimitJDK,
                                            cpulimit, connlimit,
  ...
  private static boolean loadIdentity(Configuration config) {
    ...
    try {
      ...
      if (value != null && (!value.isEmpty())) {
        ...
        logger.error(
            Messages.getString("FileBasedConfiguration.NotFoundConfig") +
            "Host ID"); //$NON-NLS-1$
        return false;
      }
    ...
    try {
      ...
      if (value == null || (value.isEmpty())) {
        ...
        try {
          ...
          logger.info(
              "Database connection: Admin:" + (DbConstant.admin != null) +
              " NoCommitAdmin:"
              + (DbConstant.noCommitAdmin != null));

          try {
            logger.info("DefaultTransactionIsolation: " +
                        DbConstant.admin.getSession().getConn().getMetaData()
  ...
  private static boolean loadDirectory(Configuration config) {
    ...
    try {
      ...
      try {
        ...
        logger.error(Messages.getString("FileBasedConfiguration.NoSetConfig") +
                     "Home"); //$NON-NLS-1$
        return false;
      }
      try {
        config.setConfigPath(FilesystemBasedDirImpl
                                 .normalizePath(
                                     getSubPath(config, XML_CONFIGPATH)));
      ...
      try {
        config.setWorkingPath(FilesystemBasedDirImpl
                                  .normalizePath(
                                      getSubPath(config, XML_WORKINGPATH)));
      } catch (OpenR66ProtocolSystemException e2) {
        logger.error(Messages.getString("FileBasedConfiguration.NoSetConfig") +
                     "Working"); //$NON-NLS-1$
        return false;
      ...
      try {
        config.setArchivePath(FilesystemBasedDirImpl
                                  .normalizePath(
                                      getSubPath(config, XML_ARCHIVEPATH)));
      } catch (OpenR66ProtocolSystemException e2) {
        logger.error(Messages.getString("FileBasedConfiguration.NoSetConfig") +
                     "Archive"); //$NON-NLS-1$
        return false;
  ...
  private static boolean loadServerParam(Configuration config) {
    ...
    try {
      ...
      if (value != null && (!value.isEmpty())) {
        ...
        logger
            .error(Messages.getString("FileBasedConfiguration.NotFoundConfig") +
      ...
      if (path == null || path.isEmpty()) {
        logger.error(
            Messages.getString("FileBasedConfiguration.NoSetConfig") +
            "Http Admin Base"); //$NON-NLS-1$
        return false;
      }
      File file = new File(path);
      if (!file.isDirectory()) {
        logger.error(
            Messages.getString("FileBasedConfiguration.NotDirectory") +
            "Http Admin Base"); //$NON-NLS-1$
        return false;
      }
      try {
      ...
      int model =
          (!new File(file, HttpResponsiveSslHandler.LISTING_PAGE).isFile())? 0 :
              1;
      if (value != null && (!value.isEmpty())) {
      ...
      if (value != null && (!value.isEmpty())) {
        ...
        if (config.getMultipleMonitors() > 1) {
          logger.warn(
              Messages.getString("FileBasedConfiguration.MMOn") //$NON-NLS-1$
  ...
  private static boolean loadSsl(Configuration config) {
    ...
    try {
      ...
      NetworkSslServerInitializer
          .setWaarpSslContextFactory(new WaarpSslContextFactory(
              NetworkSslServerInitializer.getWaarpSecureKeyStore()));
      return true;
    } finally {
      hashConfig.clear();
      ...
      if (DbConstant.admin != null) {
        try {
          DbHostConfiguration hostconfiguration =
              new DbHostConfiguration(config.getHOST_ID());
  ...
  private static void setSelfVersion(Configuration config) {
    if (config.getHOST_ID() != null) {
      config.getVersions().putIfAbsent(config.getHOST_ID(),
                                       new PartnerConfiguration(
                                           config.getHOST_ID()));
    }
    if (config.getHOST_SSLID() != null) {
      config.getVersions().putIfAbsent(config.getHOST_SSLID(),
                                       new PartnerConfiguration(
                                           config.getHOST_SSLID()));
    }
        ...
        for (XmlValue[] xml : (List<XmlValue[]>) valueRest.getList()) {
          ...
          if (config.REST_PORT > 0) {
              ...
              if (valueKey != null && (!valueKey.isEmpty())) {
                ...
                if (!file.canRead()) {
                  file = new File(
                      ...
                      FilesystemBasedDirImpl.SEPARATOR + fileKey);
                  if (!file.canRead()) {
            ...
            if (valueMethod != null && (valueMethod.getList() != null)) {
              ...
              for (XmlValue[] xmlmethod : (List<XmlValue[]>) valueMethod
                  .getList()) {
                ...
                found = true;
                byte def = 0x0;
                def |= (crud.contains("C")? RestConfiguration.CRUD.CREATE.mask :
                    0);
                def |=
                    (crud.contains("R")? RestConfiguration.CRUD.READ.mask : 0);
                def |= (crud.contains("U")? RestConfiguration.CRUD.UPDATE.mask :
                    0);
                def |= (crud.contains("D")? RestConfiguration.CRUD.DELETE.mask :
                    0);
  ...
  private static boolean loadClientParam(Configuration config) {
    ...
    try {
      XmlValue value = hashConfig.get(XML_SAVE_TASKRUNNERNODB);
      if (DbConstant.admin == null ||
          DbConstant.admin.getTypeDriver() == DbType.none) {
        if (value != null && (!value.isEmpty())) {
          config.setSaveTaskRunnerWithNoDb(value.getBoolean());
          logger.info(
              Messages.getString("FileBasedConfiguration.NoDB")); //$NON-NLS-1$
