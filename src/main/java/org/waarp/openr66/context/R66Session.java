...
public class R66Session implements SessionInterface {
  ...
  /**
   * Finalize the transfer step by running the error or post operation according
   * to the status.
   *
   * @param status
   * @param finalValue
  ...
  public void setFinalizeTransfer(boolean status, R66Result finalValue)
    ...
    if (runner.isAllDone()) {
      logger.debug(
          "Transfer already done but " + status + " on " + file +
    ...
    if (localChannelReference.getFutureRequest().isDone()) {
      logger.debug(
          "Request already done but " + status + " on " + file +
          runner.toShortString(),
          new OpenR66RunnerErrorException(finalValue.toString()));
  ...
  public void setNoSessionRunner(DbTaskRunner runner,
    ...
    this.auth
        .specialNoSessionAuth(false, Configuration.configuration.getHOST_ID());
   ...
   *     If False, no new file will be created, thus
   *     having an Exception.
   *
   * @throws OpenR66RunnerErrorException
   * @throws CommandAbstractException only when new received created file
   *     cannot be created
   */
  public void setFileAfterPreRunner(boolean createFile)
    ...
    if (runner.isSender() && file != null) {
      logger.debug(
          "could change size: " + this.runner.getOriginalSize() + " => " +
          file.length());
  ...
  public void startup(boolean checkNotExternal)
    ...
    if (runner.getGloballaststep() == TASKSTEP.TRANSFERTASK.ordinal()) {
      ...
      if (!this.runner.isSender()) {
        ...
        try {
          this.localChannelReference.getFutureRequest()
  ...
  public void setFileBeforePreRunner() throws OpenR66RunnerErrorException {
    ...
    try {
      file = FileUtils.getFile(logger, this, this.runner.getOriginalFilename(),
                               this.runner.isPreTaskStarting(),
                               this.runner.isSender(),
  ...
  public void setLocalChannelReference(
    ...
    if (this.localChannelReference.getNetworkChannel() != null) {
      this.raddress =
          this.localChannelReference.getNetworkChannel().remoteAddress();
      this.laddress =
          this.localChannelReference.getNetworkChannel().localAddress();
    } else {
