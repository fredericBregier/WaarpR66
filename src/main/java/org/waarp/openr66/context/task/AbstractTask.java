...
public abstract class AbstractTask implements Runnable {
  ...
  /**
   * Current full path of Original FILENAME (as transmitted) (before changing in
   * retrieval part)
   */
   ...
   * configuration (default no usage of LocalExec)
   */
  ...
  /**
   * @param arg as the Format string where FIXED items will be replaced by
   *     context values and next using argFormat as
   *     format second argument; this arg comes from the rule itself
   * @param argFormat as format second argument; this argFormat comes from
  ...
  protected String getReplacedValue(String arg, Object[] argFormat) {
    ...
    if (session.getRemoteAddress() != null) {
      WaarpStringUtils
          .replaceAll(builder, REMOTEHOSTADDR, session.getRemoteAddress()
                                                      .toString());
      WaarpStringUtils
          .replaceAll(builder, LOCALHOSTADDR, session.getLocalAddress()
                                                     .toString());
    } else {
    ...
    if (session.getLocalChannelReference() == null) {
      ...
      try {
        WaarpStringUtils
            .replaceAll(builder, ERRORMSG, session.getLocalChannelReference()
                                                  .getErrorMessage());
      ...
      try {
        WaarpStringUtils
            .replaceAll(builder, ERRORCODE, session.getLocalChannelReference()
