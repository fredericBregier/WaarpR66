 ...
 * according to the following argument:<br> -
 * the full path is get from the current file<br> - the arg path is transformed
 * as usual (static and dynamic from
 * information transfer)<br> - this final path arg should be of the form
 * [ua][+-=][rwx] where multiple elements could be
 * specified, separated by blank character<br> - u/a meaning user(Waarp system
 * user)/all (group and other do not exist
 * in Java), +/-/= meaning add/remove/set (set means all other values are
 * removed), r/w/x meaning
 ...
 * access mode, and no access mode (-rwx) will
 * be the default for "all" access mode.<br> If several access mode are set in
 * sequence, the result will be the sum of
 * the results, step by step.<br> "a=r a+w a-r" will result in a=w (first step
 * is r--, second step is rw-, third step is
 * -w-)
 *
...
public class ChModTask extends AbstractTask {
  ...
  public void run() {
    ...
    finalname = R66Dir.normalizePath(
        getReplacedValue(finalname, argTransfer.split(" "))).trim()
                      .toLowerCase();
