 ...
 * <p>
 * The output is ignored if the command has a correct status.<br> In case of
 * error, if the output finishes with
 * <tt>NEWFINALNAME:xxx</tt> then this part is removed from the output and the
 * xxx is used as the last valid name for
 ...
 * the feedback in case of error. So it is
 * ignored.
 *
...
public class ExecOutputTask extends AbstractExecTask {
  ...
  public void run() {
    ...
    if (Configuration.configuration.isUseLocalExec() && useLocalExec) {
      ...
      if (localExecClient.connect()) {
        localExecClient
            .runOneCommand(finalname, delay, waitForValidation,
                           futureCompletion);
    ...
    Thread thread = new Thread(allLineReader, "ExecRename" + session.getRunner()
                                                                    .getSpecialId());
  ...
  private void finalize(int status, String newName, String commandLine) {
    ...
    if (status == 0) {
      ...
      R66Result result = new R66Result(session, true, ErrorCode.Warning,
                                       this.session.getRunner());
      result.setOther(newName);
      futureCompletion.setResult(result);
  ...
  private void finalizeFromError(PipedOutputStream outputStream,
    ...
    OpenR66RunnerErrorException exc =
        new OpenR66RunnerErrorException(
            "<STATUS>" + status + "</STATUS><ERROR>" + result
            + "</ERROR>");
    futureCompletion.setFailure(exc);
  }
}
