 ...
 * following argument:<br> - SIZE
 * LT/GT/LTE/GTE/EQ number : to check the file size according to a limit (less
 * than, greater than, less than or equal,
 * greater than or equal, equal)<br> - DFCHECK : to check if the future received
 * file size is compatible with the space
 * left on both working and received directories (from general configuration or
 * rule configuration)<br>
 * <br>
 * For instance "SIZE LT 10000000 SIZE GT 1000 DFCHECK" will test that the
 * current file size is less than 10 MB (base
 * 10), and greater than 1000 bytes, and that the working and received
 * directories have enough space to receive the
 * file.
 *
      ...
      if (commands[current].equalsIgnoreCase(FC_COMMAND.SIZE.name())) {
        ...
        if (freesize > 0 && supposelength > freesize) {
          ...
          futureCompletion.setResult(
              new R66Result(session, false, ErrorCode.SizeNotAllowed, runner));
          futureCompletion
              .setFailure(new OpenR66RunnerException(
