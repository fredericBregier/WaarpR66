...
public interface R66Runnable extends Runnable {
   ...
   * @param waitForValidation True if the caller will wait up to delay time
   *     in ms
   * @param useLocalExec True if currently R66 is configured to use
   *     LocalExec (may be ignored)
   * @param delay Delay in ms used only if waitForValidation is True
   * @param classname The Classname
   * @param arg full argument
   * @param callFromBusiness True if called from Business operation, not
   *     task
