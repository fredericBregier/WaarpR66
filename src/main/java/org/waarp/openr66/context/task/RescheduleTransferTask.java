 ...
 * milliseconds, if the error code is one of the
 * specified codes and the optional intervals of date are compatible with the
 ...
 * of the current transfer (either literal
 * or code in 1 character:<br> ConnectionImpossible(C), ServerOverloaded(l),
 * BadAuthent(A), ExternalOp(E),
 * TransferError(T), MD5Error(M), Disconnection(D), RemoteShutdown(r),
 * FinalOp(F), Unimplemented(U), Shutdown(S),
 * RemoteError(R), Internal(I), StoppedTransfer(H), CanceledTransfer(K),
 * Warning(W), Unknown(-),
 * QueryAlreadyFinished(Q), QueryStillRunning(s), NotKnownHost(N),
 * QueryRemotelyUnknown(u), FileNotFound(f),
 * CommandNotFound(c), PassThroughMode(p)<br>
 * <br>
 * "-between start;end" and/or "-notbetween start;end" (multiple times are
 * allowed, start or end can be not set) and
 * where start and stop are in the following format:<br> Yn:Mn:Dn:Hn:mn:Sn where
 * n is a number for each time
 * specification, each specification is optional, as Y=Year, M=Month, D=Day,
 * H=Hour, m=minute, s=second.<br> Format can
 * be X+n, X-n, X=n or Xn where X+-n means adding/subtracting n to current date
 * value, while X=n or Xn means setting
 * exact value<br> If one time specification is not set, it is based on the
 * current date.<br>
 * <br>
 * "-count limit" will be the limit of retry. The current value limit is taken
 * from the "transferInfo" internal code
 * (not any more the "information of transfer")and not from the rule as
 * "{"CPTLIMIT": limit}" as JSON code. Each time
 * this function is called, the limit value will be replaced as newlimit = limit
 * - 1 in the "transferInfo" as
 * "{"CPTLIMIT": limit}" as JSON code.<br> To ensure correctness, the value must
 * be in the "transferInfo" internal code
 * since this value will be changed statically in the "transferInfo". If taken
 * from the rule, it will be wrong since the
 * value will never decrease. However, a value must be setup in the rule in
 * order to reset the value when the count
 * reach 0. <br> So in the rule, "-count resetlimit" must be present, where
 * resetlimit will be the new value set when
 * the limit reach 0, and in the "transferInfo" internal code, "{"CPTLIMIT":
 * limit}" as JSON code must be present. If
 * one is missing, the condition is not applied. <br>
 * <br>
 * If "-notbetween" is specified, the planned date must not be in the area.<br>
 * If "-between" is specified, the planned
 * date must be found in any such specified areas (could be in any of the
 * occurrence). If not specified, it only depends
 * on "-notbetween".<br> If none is specified, the planned date is always
 * valid.<br>
 * <br>
 * <p>
 * Note that if a previous called to a reschedule was done for this attempt and
 * was successful, the following calls will
 * be ignored.<br>
 * <br>
 * <B>Important note: any subsequent task will be ignored and not executed once
 * the reschedule is accepted.</B><br>
 * <br>
 * In case start > end, end will be +1 day<br> In case start and end < current
 * planned date, both will have +1 day.<br>
 * <br>
 * <p>
 * Example: -delay 3600000 -case ConnectionImpossible,ServerOverloaded,Shutdown
 * -notbetween H7:m0:S0;H19:m0:S0
 * -notbetween H1:m0:S0;H=3:m0:S0<br> means retry in case of error during
 * initialization of connection in 1 hour if not
...
public class RescheduleTransferTask extends AbstractTask {
  /**
   * Delimiter for -count option in Reschedule to be placed in the transfer info
   * of transfer as {"CPTLIMIT": limit}
  ...
  protected boolean validateArgs(String[] args) {
    ...
    for (int i = 0; i < args.length; i++) {
      if (args[i].equalsIgnoreCase("-notbetween")) {
        ...
        logger
            .debug("Dates after check: Not between " + start.getTime() + " and "
                   + stop.getTime());
        if (!startModified) {
  ...
  /**
   * @param values as X+n or X-n or X=n or Xn where X=Y/M/D/H/m/s, n=number
   *     and +/- meaning adding/subtracting from
