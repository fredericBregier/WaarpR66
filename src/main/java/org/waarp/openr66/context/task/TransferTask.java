 ...
 * Transfer task:<br>
 * <p>
 * Result of arguments will be as r66send command.<br> Format is like r66send
 * command in any order except "-info" which
 * should be the last item, and "-copyinfo" will copy at first place the
 * original transfer information as the new one,
 * while still having the possibility to add new informations through
 * "-info":<br> "-file filepath -to requestedHost
 * -rule rule [-md5] [-start yyyyMMddHHmmss or -delay (delay or +delay)]
...
public class TransferTask extends AbstractExecTask {
  ...
  public void run() {
    ...
    SubmitTransfer transaction = new SubmitTransfer(future,
                                                    requested, filepath, rule,
                                                    finalInformation, isMD5,
                                                    blocksize,
