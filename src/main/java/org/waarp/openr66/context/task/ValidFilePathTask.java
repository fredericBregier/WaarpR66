 ...
 * the full path is get from the current
 * file<br> - the arg path is transformed as usual (static and dynamic from
 * information transfer) and should be the
 * beginning of the correct valid path<br> - the full path should begin with one
 * of the result of the transformation
 * (blank separated)<br>
 * <br>
 * For instance "#OUTPATH# #INPATH# #WORKPATH# #ARHCPATH#" will test that the
 * current file is in one of the standard
 * path.
 *
  ...
  public void run() {
    ...
    for (String base : paths) {
      if (curpath.startsWith(base)) {
        if (delay > 0) {
          logger
              .info("Validate File " + curpath + " from " + base + " and     " +
                    session.toString());
        }
