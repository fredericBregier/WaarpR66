 ...
 * <p>
 * <p>
 * Several kind of tasks exist in OpenR66:<br> LOG, MOVE, MOVERENAME, COPY, COPYRENAME, EXEC, EXECMOVE, LINKRENAME,
 * TRANSFER, VALIDFILEPATH, DELETE<br>
 * <br>
 * <br>
 * <br>
 * <p>
 * Several tasks are possible to run before a transfer starts (pre action), after a transfer is finished correctly (post
 * action) or after an error occurs (either in pre or post action or during transfer: error action).
 * <p>
 ...
 * - if delay is 0, no echo at all will be done<br> - if delay is 1, will echo some information in the normal log<br> -
 * if delay is 2, will echo some information in the file (last deduced argument will be the full path for the file
 * output)<br> - if delay is 3, will echo both in the normal log and in the file (last deduced argument will be the full
 ...
 * arguments. Delay is ignored. After Path is transformed according to above dynamic replacements, it is then used as a
 * String Format where Transfer Information is used as input (String.format(Path,Info)). The file is marked as moved.
 ...
 * arguments. Delay is ignored. After Path is transformed according to above dynamic replacements, it is then used as a
 * String Format where Transfer Information is used as input (String.format(Path,Info)). The file is not marked as
 * moved.
 * <li>EXEC</li> Execute an external command given by Path and Transfer Information arguments. The
 * Delay is the maximum amount of time in milliseconds before the task should be considered as over time and so in
 * error.<br> The command path is obtained from Path transformed according to above dynamic replacements, and after a
 * String Format where Transfer Information is used as input (String.format(Path,Info)).
 * <li>EXECMOVE</li> Execute an external command given by Path and Transfer Information arguments.
 * The Delay is the maximum amount of time in milliseconds before the task should be considered as over time and so in
 * error.<br> The command path is obtained from Path transformed according to above dynamic replacements, and after a
 * String Format where Transfer Information is used as input (String.format(Path,Info)).<br> The last line returned by
 * the external command is interpreted as the new full file path. The external command is responsible to really move the
 * previous file to the new one. The file is marked as moved.
 * <li>TRANSFER</li> Submit a new transfer based on the Path and Transfer Information arguments. The
 * transfer arguments are obtained from Path transformed according to above dynamic replacements, and after a String
 * Format where Transfer Information is used as input (String.format(Path,Info)). The result should be as r66send except
 * that "-info" must be the last entry:<br> "-file filepath -to requestedHost -rule rule [-md5] -info
 * transferInformation" where each field is separated by blank character. Last field (transferInformation) may contain
 * however blank character. Delay is ignored. The file is not marked as moved.
 * <li>VALIDFILEPATH</li> Test if the current file is under one of the paths based on the Path and
 * Transfer Information arguments. The paths arguments are obtained from Path transformed according to above dynamic
 * replacements, and after a String Format where Transfer Information is used as input (String.format(Path,Info)). The
 * result should be as: "path1 path2 ..." where each path is separated by blank character. If Delay is not 0, a log is
 * printed out. The file is not marked as moved.
 ...
 * first tries to be a hard link, then a soft link, and if it is really not possible (not supported by the filesystem),
 * it does a copy and rename task.<br> Delay is ignored. After Path is transformed according to above dynamic
 * replacements, it is then used as a String Format where Transfer Information is used as input
