...
public class R66SystemProperties {
   ...
   * occurs but an error in "pre-task" occurs:
   * default true
   */
  ...
  /**
   * Shall we print info of startup in Warning mode (true, default) or Info mode
   * (false)
   */
   ...
   * Rule Out directory (default = false)
   */
   ...
   * mandatory false). Note that this must not be true if several partners might
  ...
  /**
   * Filename max length (default = 255), only for Basename (not full path). to
   * choose the default max filename length
   * used when receiving a file (for the temporary filename and final filename).
   * This does not prevent to change the
   * filename after (and #ORIGINALFILENAME# does still contain the full
   ...
   * value in the definition. If 0 or less,
   * means not activated.
   */
  public static final String OPENR66_TRACE_STATS = "openr66.trace.stats";
  /**
   * Maximum number of DbTaskRunners to keep in LRU cache (used in self request
   * and for instance without database).
   * Minimal value is 100.
   */
  public static final String OPENR66_CACHE_LIMIT = "openr66.cache.limit";
  /**
   * Maximum time in ms of valid element once created, used or updated (used in
   * self request and for instance without
   * database). Minimal value is 1000 ms (1s). If set to 1000, the value will
