 ...
 */
public class HttpResponsiveSslHandler
   ...
   * among all associated LocalChannels in the
   * session
   */
  ...
  private String resetOptionTransfer(String header, String startid,
                                     String stopid,
                                     String start, String stop, String rule,
                                     String req,
                                     boolean pending, boolean transfer,
                                     boolean error, boolean done, boolean all) {
  ...
  private String ListingReload() {
    ...
    if (parms != null) {
      ...
      if ("Filter".equalsIgnoreCase(parm) || !isNotReload) {
        ...
        Long idstart = null;
        head = setDbTaskRunnerJsonData(head, errorText, startid, stopid, tstart,
                                       tstop, rule, req, pending,
                                       transfer, error, done, all);
        head = resetOptionTransfer(head, startid == null?
                                       (idstart != null? idstart.toString() : "")
                                       : startid,
  ...
  private String CancelRestart() {
    ...
    if (params == null) {
      ...
      head = resetOptionTransfer(head, "", "", "", "", "", "",
                                 false, false, false, false, true);
      head =
          setDbTaskRunnerJsonData(head, "", "", "", null, null, "", "", false,
    ...
    if (parms != null) {
      ...
      if ("Search".equalsIgnoreCase(parm)) {
        ...
        head = setDbTaskRunnerJsonData(head, errorText, startid, stopid, null,
                                       null, null, null, false, false,
                                       false, false, true);
        head = resetOptionTransfer(head, startid == null? "" : startid,
                                   stopid, "", "", "", "", false, false, false,
                                   false, true);
          ...
          try {
            ...
            while (preparedStatement.getNext()) {
              try {
                ...
                R66Result finalResult =
                    TransferUtils.restartTransfer(taskRunner,
                                                  lcr);
  ...
  private String Export() {
      ...
      getValid =
          DbTaskRunner
              .getFilterPrepareStatement(dbSession, 0,// 0 means no limit
                                         true, null, null, tstart, tstop, rule,
                                         req,
                                         pending, transfer, error, done, all,
                                         seeAll);
    ...
    return body
               .replace(
                   ...
                   "Export "
                   + (isexported?
                       "<B>" + Messages.getString("HttpSslHandler.8") +
                       //$NON-NLS-1$
                       "<A href='" + basename + "' target='_blank'>" +
                       basename + "</A>"
                       + Messages.getString("HttpSslHandler.9") +
                       nbAndSpecialId.nb +
                       Messages.getString("HttpSslHandler.10") + purge
                       //$NON-NLS-1$ //$NON-NLS-2$
                       + Messages.getString("HttpSslHandler.11") + "</B>" :
                       //$NON-NLS-1$
                       "<B>" + Messages.getString("HttpSslHandler.12"))) +
           "</B>" +
           errorMsg; //$NON-NLS-1$
  }
  ...
  private String Hosts() {
    ...
    if (parms != null) {
      ...
      if ("Create".equalsIgnoreCase(parm)) {
        ...
        boolean resultShutDown = NetworkTransaction
  ...
  private String resetOptionRules(String header,
                                  String rule, RequestPacket.TRANSFERMODE mode,
                                  int gmode) {
    StringBuilder builder = new StringBuilder(header);
    WaarpStringUtils.replace(builder, "XXXRULEXXX", rule);
    if (mode != null) {
    ...
    if (parms != null) {
      ...
      if ("Create".equalsIgnoreCase(parm) || "Update".equalsIgnoreCase(parm)) {
        ...
        if (rule == null || mode == null) {
          errorText = Messages.getString("HttpSslHandler.26") + parm +
                      Messages.getString(
        ...
        logger.debug("Recv UpdOrInsert: " + rule + ":" + hostids + ":" +
                     tmode.ordinal() + ":" +
                     recvp + ":" + sendp + ":" + archp + ":" + workp + ":" +
                     rpre + ":" + rpost + ":" + rerr +
                     ":"
                     + spre + ":" + spost + ":" + serr);
        DbRule dbrule = new DbRule(rule, hostids, tmode.ordinal(),
                                   recvp, sendp, archp, workp, rpre, rpost,
                                   rerr, spre, spost, serr);
        try {
          if ("Create".equalsIgnoreCase(parm)) {
            dbrule.insert();
          } else {
    ...
    if (params.containsKey("ACTION")) {
      ...
      for (String act : action) {
        if (act.equalsIgnoreCase("Language")) {
          ...
          String base =
              Configuration.configuration.getBaseDirectory() + R66Dir.SEPARATOR;
          String directory =
          ...
          extraInformation = Messages.getString("HttpSslHandler.ExportDir") +
                             ...
                             "<br>"; //$NON-NLS-1$
          String[] filenames = ServerActions
          ...
          if (Configuration.configuration
            ...
            error = error(Messages.getString("HttpSslHandler.39") +
                          (Configuration.configuration.getTIMEOUTCON() * 2 /
                           1000) +
                          Messages.getString(
                              "HttpSslHandler.40")); //$NON-NLS-1$ //$NON-NLS-2$
          }
          error = error
              .replace("XXXRELOADHTTPXXX", "HTTP-EQUIV=\"refresh\" CONTENT=\""
                                           +
                                           (Configuration.configuration
                                                .getTIMEOUTCON() * 2 / 1000) +
                                           "\"");
  ...
  protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg)
    ...
    if (uriRequest.contains("gre/") || uriRequest.contains("img/") ||
        ...
        uriRequest.contains("js/") ||
        uriRequest.contains("datatable/") ||
  ...
  private static enum REQUEST {
    ...
    public String read(HttpResponsiveSslHandler handler) {
      return handler.readFileHeader(
          Configuration.configuration.getHttpBasePath() + this.header);
    }
  }

  private static enum REPLACEMENT {
    XXXHOSTIDXXX, XXXADMINXXX, XXXVERSIONXXX, XXXBANDWIDTHXXX,
    XXXBANDWIDTHINXXX, XXXBANDWIDTHOUTXXX,
