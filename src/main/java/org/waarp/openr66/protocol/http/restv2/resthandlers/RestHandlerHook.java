 ...
 * AbstractRestDbHandler} when a request is
 * made. These hooks check the user authentication and privileges, as well as
...
public class RestHandlerHook implements HandlerHook {
  ...
  /**
   * Creates a HandlerHook which will check for authentication and signature on
   * incoming request depending on the
   * parameters.
   *
   ...
   * is active in the CRUD configuration,
   * checks the request's content type, and finally checks the user
   ...
   *
   * @return {@code true} if the request can be handed to the handler, or {@code
   *     false} if an error occurred and a
   ...
   * chosen to process the request. This is
   ...
   *
   * @throws IllegalArgumentException if the given method name does not
   *     exist
   */
   ...
   * @throws NotAllowedException if the user making the request does not
   *     exist
   */
   ...
   *
   * @return {@code true} if the user is authorized to make the request, {@code
   *     false} otherwise.
   */
   ...
   * Method}. This list is based on the types
   * indicated by the method's {@link Consumes} annotation. If the annotation is
   * absent, the method will be assumed to
   * accept any type.
   *
  ...
  private List<MediaType> getExpectedMediaTypes(Method method) {
    ...
    if (method.isAnnotationPresent(Consumes.class)) {
      ...
      logger.warn(
          String.format("[RESTv2] The method %s of handler %s is missing " +
   ...
   * content type of the method called. If
   * no content type header can be found, the request will be assumed to have a
