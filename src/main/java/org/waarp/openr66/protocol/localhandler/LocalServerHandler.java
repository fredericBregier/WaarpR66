    ...
    if (packet.getType() == LocalPacketFactory.STARTUPPACKET) {
      ...
      case LocalPacketFactory.DATAPACKET: {
        if (((DataPacket) packet).getPacketRank() % 100 == 1
            || serverHandler.getSession().getState() !=
               R66FiniteDualStates.DATAR) {
      ...
      case LocalPacketFactory.VALIDPACKET: {
        ...
        if (((ValidPacket) packet).getTypeValid() ==
          ...
          if (newFileInfo != null &&
              !newFileInfo.equals(
                  serverHandler.session.getRunner().getFileInformation())) {
            serverHandler.requestChangeFileInfo(ctx.channel(), newFileInfo);
          }
          serverHandler
              .requestChangeNameSize(ctx.channel(), newfilename, newSize);
          packet.clear();
        } else {
      ...
      case LocalPacketFactory.REQUESTPACKET: {
        serverHandler
            .request((LocalChannel) ctx.channel(), (RequestPacket) packet);
        break;
      }
      ...
      case LocalPacketFactory.BANDWIDTHPACKET: {
        ...
        serverHandler.getLocalChannelReference()
                     .invalidateRequest(new R66Result(
                         new OpenR66ProtocolSystemException(
                             "Not implemented"), serverHandler.getSession(),
                         true,
        ...
        final ErrorPacket errorPacket = new ErrorPacket(
            ...
            ErrorPacket.FORWARDCLOSECODE);
        ChannelUtils
            .writeAbstractLocalPacket(serverHandler.getLocalChannelReference(),
                                      errorPacket, true)
            .addListener(
      ...
      case LocalPacketFactory.BUSINESSREQUESTPACKET: {
        serverHandler
            .businessRequest(ctx.channel(), (BusinessRequestPacket) packet);
        break;
      }
      ...
      default: {
        ...
        serverHandler.getLocalChannelReference()
                     .invalidateRequest(new R66Result(
                         new OpenR66ProtocolSystemException(
                             "Unknown Message"), serverHandler.getSession(),
                         true,
                         ErrorCode.Unimplemented, null));
        final ErrorPacket errorPacket = new ErrorPacket(
  ...
  public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
    ...
    if (exception != null) {
      ...
      if (exception instanceof OpenR66ProtocolShutdownException) {
        ...
        R66Result finalValue =
            new R66Result(
                exception, serverHandler.getSession(), true, code,
                serverHandler.getSession()
                             .getRunner());
        try {
          serverHandler.getSession().setFinalizeTransfer(false, finalValue);
