 ...
 * bandwidth control, configuration import/export,
 * log purge, request restart/stop/cancel, business request, block new request
 * control, information request and transfer
 * request.
 * <p>
...
public class ServerActions extends ConnectionActions {
   ...
   * @param isTo True for remote host is requester, False for requested
   *     (default)
   * @param remoteHost requester/requested for transfer if jsonOutput is
   *     True, else (jsonOutput False) remoteHost is
  ...
  public final ValidPacket informationRequest(long id, boolean isTo,
    ...
    if (isTo) {
      try {
        ...
        logger.debug(
            "RunnerTask is not found: " + id + ":" + remote + ":" + local, e);
        throw new OpenR66ProtocolNoDataException(
            Messages.getString("LocalServerHandler.22") + id,
            e); //$NON-NLS-1$
      }
    } else {
      try {
        ...
        logger.debug(
            "RunnerTask is not found: " + id + ":" + local + ":" + remote, e);
        logger.error(Messages.getString("LocalServerHandler.21") + id);
        throw new OpenR66ProtocolNoDataException(
            "(Local) " + Messages.getString("LocalServerHandler.21") + id,
            e);
      }
    }
    if (!jsonOutput) {
  ...
  public final ValidPacket informationFile(byte request, String rulename,
    ...
    try {
      ...
      if (request == InformationPacket.ASKENUM.ASKLIST.ordinal() ||
        ...
        ValidPacket validPacket =
            new ValidPacket(builder.toString(), "" + list.size(),
                            LocalPacketFactory.INFORMATIONPACKET);
        R66Result result = new R66Result(session, true,
                                         ErrorCode.CompleteOk, null);
        result.setOther(validPacket);
  ...
  public void valid(Channel channel, ValidPacket packet)
    ...
    case LocalPacketFactory.REQUESTUSERPACKET: {
      ...
      R66Result resulttest = new R66Result(session, true,
                                           ErrorCode.getFromCode(
                                               packet.getSmiddle()), null);
      resulttest.setOther(packet);
  ...
  public final R66Result requestRestart(String reqd, String reqr, long id,
    ...
    if (Configuration.configuration.getConstraintLimitHandler()
      ...
      if (reqd == null || reqr == null || id == DbConstant.ILLEGALVALUE) {
        ...
        try {
          DbSession dbSession = (localChannelReference != null)?
              localChannelReference.getDbSession()
              : DbConstant.admin.getSession();
  ...
  public final String[] configExport(boolean bhost, boolean brule,
    ...
    DbSession dbSession =
        (localChannelReference != null)? localChannelReference.getDbSession()
            : DbConstant.admin.getSession();
    return staticConfigExport(dbSession, dir, bhost, brule, bbusiness, balias,
                              broles);
  }
   ...
   */
  private final boolean stopOrCancelRunner(long id, String reqd, String reqr,
                                           ErrorCode code) {
    try {
      DbSession dbSession =
          (localChannelReference != null)? localChannelReference.getDbSession()
   ...
   * @return filenames in order
   */
  public static String[] staticConfigExport(DbSession dbSession, String dir,
                                            boolean bhost, boolean brule,
                                            boolean bbusiness, boolean balias,
                                            boolean broles) {
    String shost = null, srule = null, sbusiness = null, salias = null, sroles =
        null;
    ...
    if (bhost) {
      ...
      try {
        AuthenticationFileBasedConfiguration
            .writeXML(Configuration.configuration,
                      filename);
        shost = filename;
      } catch (WaarpDatabaseNoConnectionException e) {
        logger.error("Error", e);
  ...
  public void jsonCommand(Channel channel, JsonCommandPacket packet)
    ...
    case LocalPacketFactory.CONFEXPORTPACKET: {
      ...
      // Now answer
      ConfigExportResponseJsonPacket resp =
    ...
    case LocalPacketFactory.CONFIMPORTPACKET: {
      ConfigImportResponseJsonPacket resp =
   ...
   * as status, channel closing if needed or
   * not.
   *
   * @param isToApplied True means this is an action request, False it is
   *     the feedback
   * @param className
   * @param arguments
  ...
  public final R66Future businessRequest(boolean isToApplied, String className,
    ...
    if (argTransfer && !Configuration.configuration.getBusinessWhiteSet()
  ...
  public final String[] logPurge(boolean purge, boolean clean,
                                 ...
                                 String startid, String stopid, String rule,
                                 String request,
                                 boolean pending, boolean transfer,
                                 boolean done, boolean error, boolean isPurge)
      throws OpenR66ProtocolNotAuthenticatedException,
             OpenR66ProtocolBusinessException {
    // should be from the local server or from an authorized hosts: LOGCONTROL
    try {
      ...
      getValid =
          DbTaskRunner
              .getFilterPrepareStatement(dbSession, 0,// 0 means no limit
                                         true, startid, stopid, start, stop,
                                         rule, request,
  ...
  public final ConfigImportResponseJsonPacket configImport(
    ...
    boolean importedhost = false, importedrule = false, importedbusiness =
        false, importedalias = false,
        importedroles = false;
    ...
    if (sbusiness != null || salias != null || sroles != null ||
        ...
        || ((businessid != DbConstant.ILLEGALVALUE ||
             aliasid != DbConstant.ILLEGALVALUE ||
   ...
   */
  private final DbTaskRunner initTransferRequest(
      TransferRequestJsonPacket request) {
    ...
    if (request.getSpecialId() != 0 ||
        request.getSpecialId() == DbConstant.ILLEGALVALUE) {
  ...
  public void blockRequest(Channel channel, BlockRequestPacket packet)
    ...
    ValidPacket valid = new ValidPacket(
