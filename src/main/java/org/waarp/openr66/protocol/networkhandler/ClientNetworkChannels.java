 ...
 * client to the current Host. As one Id can
 * be shared or one can use direct send, so having a connection by request, this
 * class is useful when one wants to know
...
public class ClientNetworkChannels {
  ...
  private final Set<NetworkChannelReference> networkChannelReferences =
      new ConcurrentSet<NetworkChannelReference>();
