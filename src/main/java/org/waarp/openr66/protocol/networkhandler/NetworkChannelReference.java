...
public class NetworkChannelReference {
   ...
   */
  private final ConcurrentSet<LocalChannelReference> localChannelReferences =
      new ConcurrentSet<LocalChannelReference>();
  /**
  ...
  public NetworkChannelReference(Channel networkChannel, WaarpLock lock) {
    ...
    this.hostAddress =
        ((InetSocketAddress) this.networkAddress).getAddress().getHostAddress();
    this.lock = lock;
  ...
  public void shutdownAllLocalChannels() {
    ...
    ArrayList<LocalChannelReference> toCloseLater =
  ...
  /**
   * Check if the last time used is ok with a delay applied to the current time
   * (timeout)
   *
   * @param delay
   *
