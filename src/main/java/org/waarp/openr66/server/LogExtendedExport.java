...
public class LogExtendedExport implements Runnable {
  ...
  public void run() {
    ...
    byte type = (purgeLog)? LocalPacketFactory.LOGPURGEPACKET :
    ...
    try {
      ...
      future.setResult(new R66Result(
          new OpenR66ProtocolNoConnectionException(
              "Cannot connect to server " + host.getHostid()),
  ...
  public void importLog(R66Future future)
    ...
    if (future.isSuccess()) {
      JsonCommandPacket packet =
          (JsonCommandPacket) future.getResult().getOther();
      if (packet != null) {
        LogResponseJsonPacket res =
        ...
        if (fileExported != null && !fileExported.isEmpty()) {
          ...
          DirectTransfer transfer =
              new DirectTransfer(futuretemp, host.getHostid(),
                                 fileExported,
